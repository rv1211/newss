<?php
class Ecomtrack extends CI_Controller
{
	...
	public function ecom_tracking_webhook()
	{
		...
		$post = file_get_contents('php://input');
		// $post = ' { "awb" : "8816899459", "datetime" : "2021-05-11 10:00:00", "status" : "013","reason_code" : "1230", "reason_code_number" : "1230", "location" : "MUMBAI", "Employee" : "ZEESHANKHANPATHAN", "status_update_number" : "148473601", "order_number" : "PDL1-1162S","city" : "MUMBAI","ref_awb" : ""}';
		...
		if (!empty($array['awb']) && !empty($array['status']) && !empty($array['status_update_number'])) {
			...
			if (!empty($single_order_info)) {
				...
				echo "<pre>";
				print_r($insert_order_tracking_detail);
				echo "</pre>";

				if ($status_data['status_id'] == '9' && $single_order_info['order_status_id'] == '5') {
					$status = $this->db->select('order_id')->from('order_airwaybill_detail')->where('airwaybill_no', $array['awb'])->where('order_status_id', '9')->get()->result_array();
					if (empty($status)) {
				...
				if ($status_data['status_id'] == '14'  &&  in_array($single_order_info['order_status_id'], array('9', '10', '11', '12'))) {
				...
				echo "<pre>";
				print_r($single_order_status_info);
				echo "</pre>";
				$insert_order_tracking_detail['scan'] = $single_order_status_info['status_name'];
				...
				$update_data = [
					...
					'remark' => $status_data['remarks'],
					'delivery_date' => ($status_data['status_id'] == '14' || $status_data['status_id'] == '6')  ?  date('Y-m-d H:i:s', strtotime($status_data['datetime'])) : null,
				];
				...
				$this->Common_model->update($update_data, 'order_airwaybill_detail', array('order_id' => $single_order_info['order_id']));
				...
				echo $awb_update_status . "<br><br>" . $tracking_status;
				...
				// dd($insert_order_tracking_detail);
			} else {
