...
class Dashboard_model extends CI_Model
{
    ...
    public function get_today_order_count($today, $fromday, $status_id, $userId, $userType)
    {
        ...
        if (!empty($status_id)) {
            $this->db->where('oad.order_status_id', $status_id);
        } else {
    ...
    public function get_year_chart_count($userType, $userId)
    {
        $where = "";
        if ($userType == 4) {
            $where = " AND fom.sender_id = $userId";
        }
        $sql = "SELECT MONTHNAME(fom.created_date) AS month_name, COUNT(fom.id) AS orders FROM forward_order_master as fom WHERE  fom.is_cancelled = '0' AND fom.is_delete= '0' AND  YEAR(fom.created_date) = YEAR(CURDATE())  $where GROUP BY MONTH(fom.created_date) ";
