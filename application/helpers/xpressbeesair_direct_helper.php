<?php
class xpressbeesair_direct
{
	/**
	 * Xpressbees API Order Create
	 * @return Response
	 */
	public static function xpressbeesair_order($order_id, $is_repeat = 0, $bulk = 0)
	{
		...
		$path = APPPATH . 'logs/xpressbees_air_order/';
		if (!file_exists($path)) {
		...
		if ($awbNumber != "") {
			...
			if ($walletSufficient == 1) {
				$body = '{
					...
					"BusinessAccountName": "Ship Secure Air",
				...
				$token = $CI->db->select('xbairtoken')->from('zship_token_master')->where('id', '1')->get()->row_array();
				$log_data['order_xpressbees_token'] = $token;
				$response = CUSTOM::curl_request('application/json', '', $CI->config->item('XPRESSBEES_ORDER_API_URL'), $body, "POST", "", "", $token['xbairtoken']);
				$log_data['order_xpressbees_curl_response'] = $response;
				file_put_contents(APPPATH . 'logs/xpressbees_air_order/' . date("d-m-Y") . '_xpressbeesair_order_log.txt', "\n" . print_r($log_data, true), FILE_APPEND);
	...
	public static function cancel_order($order_id)
	{
		...
		$cancel_order_body = ' {
				"XBkey":"' . $CI->config->item('XPRESSBEES_AIR_ORDER_KEY') . '",
		...
		file_put_contents(APPPATH . 'logs/xpressbees_air_order/' . date("d-m-Y") . '_xpressbees_cancel_order_log.txt', "\n" . print_r($log_data, true), FILE_APPEND);
		return 1;
	}
}
