...
if (!function_exists('get_shiping_price')) {
	...
	function get_shiping_price($sender_id, $logistic_ids = NUll, $from_pin, $to_pin, $shipment_type, $order_typ, $volumetric_weight, $physical_weight, $collectable_amount, $is_bulk = 0, $gst = 18)
	{
		...
		$log_data = array('date' => date('Y-m-d H:s:i'), 'sender_id' => $sender_id, 'order_type' => $order_typ, 'logistic_ids' => $logistic_ids, 'from_pin' => $from_pin, 'to_pin' => $to_pin, 'shipment_type' => $shipment_type, 'volumetric_weight' => $volumetric_weight, 'physical_weight' => $physical_weight, 'collectable_amount' => $collectable_amount, 'is_bulk' => $is_bulk, 'gst' => $gst);
		...
		function get_price_on_rules_per_logistic($physical_weight, $rules, $mange_price_type, $order_typ, $collectable_amount, $shipment_type_previus = FALSE)
		{
			...
			foreach ($rules as $key1 => $rul) {
				if ($key1 == 0 && $total_rows == 0) {
					...
					$CI->benchmark->mark('price_start');
					// echo '<pre>';
					// print_r("awailible : " . $awailable_weight);
					// echo "<br>";
					// print_r("Incremental : " . $incrimantal_weight);
					// echo "<br>";
					// print_r("Total : " . $total_weight);
					// echo '</pre>';
					// exit();
					while ($incrimantal_weight < $total_weight) {
						$CI->benchmark->mark('price_end');
					...
					// print_r("Incremental : " . $incrimantal_weight);
					// echo "<br>";
					// print_r("TO KG : " . $rul['to_kg']);
					// echo "<br>";
					// print_r("RIL : " . $rul[$mange_price_type]);
					// echo "<br>";
					// print_r("RIL : " . $total_price);
					// echo '</pre>';
